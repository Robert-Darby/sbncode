...
namespace caf
{
                       ...
                       bool allowEmpty)
  {
    ...
    for (const auto &matchedCRTHit : match.matchedCRTHits)
    {
      ...
      {
        if (matchedCRTHit.sys == 0)
          topen++;
        else if (matchedCRTHit.sys == 1)
          sideen++;
      }
                   ...
                   bool allowEmpty)
  {
    ...
    {
      if (PMT <= 89)
        sumEast += flash.PEs().at(PMT + countingOffset);
      else
        sumWest += flash.PEs().at(PMT + countingOffset);
    }
  ...
  {
    srmichel.setDefault();
    srmichel.muontime = tag.MuonTime;
    srmichel.micheltime = tag.MichelTime;
    srmichel.muonrawamp = tag.MuonRawAmp;
    srmichel.michelrawamp = tag.MichelRawAmp;
    srmichel.muonsadcwamp = tag.MuonSADCWAmp;
    srmichel.michelsadcwamp = tag.MichelSADCWAmp;
    srmichel.muong4id = tag.MuonG4ID;
                      ...
                      bool allowEmpty)
  {
    ...
    for (int p = 0; p < 3; ++p)
      srshower.plane[p].nHits = 0;
    for (auto const &hit : hits)
      ++srshower.plane[hit->WireID().Plane].nHits;
                           ...
                           caf::SRSlice &slice)
  {
    ...
    for (unsigned int i = 0; i < nHits; i++)
    {
      ...
      if (thisHit.SignalType() != geo::kCollection)
        continue;
  ...
  void FillPlaneChi2PID(const anab::ParticleID &particle_id, caf::SRTrkChi2PID &srpid)
  {
    ...
    for (size_t i_algscore = 0; i_algscore < AlgScoresVec.size(); i_algscore++)
    {
      ...
      if (AlgScore.fAlgName == "Chi2")
      {
        ...
        else if (TMath::Abs(AlgScore.fAssumedPdg) == 321)
        { // chi2ka
          srpid.chi2_kaon = AlgScore.fValue;
          srpid.pid_ndof = AlgScore.fNdf;
        }
        else if (TMath::Abs(AlgScore.fAssumedPdg) == 2212)
        { // chi2pr
                          ...
                          caf::SRTrackCalo &srcalo)
  {
    ...
    for (unsigned i = 0; i < dedx.size(); i++)
    {
      ...
      if (fill_calo_points && ((rrmax - rr[i]) < fillhit_rrstart || // near start
                               rr[i] < fillhit_rrend))
      { // near end
                   ...
                   bool allowEmpty)
  {
    ...
    srpfp.parent_is_primary = (particle.Parent() == recob::PFParticle::kPFParticlePrimary) || (primary && particle.Parent() == primary->Self());
