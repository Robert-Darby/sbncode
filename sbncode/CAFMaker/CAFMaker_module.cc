...
namespace caf
{
  ...
  void CAFMaker::InitVolumes()
  {
    ...
    {
      double XMin = std::min_element(tpcs.begin(), tpcs.end(), [](auto &lhs, auto &rhs)
                                     { return lhs.MinX() < rhs.MinX(); })
                        ->MinX();
      double YMin = std::min_element(tpcs.begin(), tpcs.end(), [](auto &lhs, auto &rhs)
                                     { return lhs.MinY() < rhs.MinY(); })
                        ->MinY();
      double ZMin = std::min_element(tpcs.begin(), tpcs.end(), [](auto &lhs, auto &rhs)
                                     { return lhs.MinZ() < rhs.MinZ(); })
                        ->MinZ();

      double XMax = std::max_element(tpcs.begin(), tpcs.end(), [](auto &lhs, auto &rhs)
                                     { return lhs.MaxX() < rhs.MaxX(); })
                        ->MaxX();
      double YMax = std::max_element(tpcs.begin(), tpcs.end(), [](auto &lhs, auto &rhs)
                                     { return lhs.MaxY() < rhs.MaxY(); })
                        ->MaxY();
      double ZMax = std::max_element(tpcs.begin(), tpcs.end(), [](auto &lhs, auto &rhs)
  ...
  void CAFMaker::beginRun(art::Run &run)
  {
    ...
    if (fParams.DetectorOverride() == "sbnd")
      override = kSBND;
    if (fParams.DetectorOverride() == "icarus")
      override = kICARUS;
    ...
    for (unsigned int i = 0; i < gdml.size(); ++i)
      gdml[i] = std::tolower(gdml[i]);
    ...
    if (fParams.SystWeightLabels().empty())
      return; // no need for globalTree
    ...
    if (fFile)
      AddGlobalTreeToFile(fFile, global);
    if (fParams.CreateBlindedCAF() && fFileb)
      AddGlobalTreeToFile(fFileb, global);
    if (fParams.CreateBlindedCAF() && fFilep)
      AddGlobalTreeToFile(fFilep, global);
    if (fFlatFile)
      AddGlobalTreeToFile(fFlatFile, global);
    if (fParams.CreateBlindedCAF() && fFlatFileb)
      AddGlobalTreeToFile(fFlatFileb, global);
    if (fParams.CreateBlindedCAF() && fFlatFilep)
      AddGlobalTreeToFile(fFlatFilep, global);
  }
  ...
  void CAFMaker::beginSubRun(art::SubRun &sr)
  {
    ...
    if (bool(bnb_spill) + bool(numi_spill) + bool(bnb_offbeam_spill) + bool(numi_offbeam_spill) > 1)
    {
      ...
      if (bnb_spill)
        std::cout << fParams.BNBPOTDataLabel() << " ";
      if (numi_spill)
        std::cout << fParams.NuMIPOTDataLabel() << " ";
      if (bnb_offbeam_spill)
        std::cout << fParams.OffbeamBNBCountDataLabel() << " ";
      if (numi_offbeam_spill)
        std::cout << fParams.OffbeamNuMICountDataLabel();
      std::cout << std::endl;
      abort();
    }
  ...
  void CAFMaker::produce(art::Event &evt) noexcept
  {
    ...
    if (mevprtltruth_handle.isValid())
    ...
    std::vector<std::string> michel_tags;
    fParams.MichelTagLabels(michel_tags);
    ...
    for (size_t i = 0; i < mctruths.size(); i++)
    {
      ...
      {
        if (!fm.isValid())
          continue; // Don't crash if StrictMode==false
    ...
    const double triggerShift = (isValidUnshiftedTrigger && isValidTrigger) ? unshifted_trig_handle->at(0).TriggerTime() - trig_handle->at(0).TriggerTime() : 0.;
    ...
    for (const std::string &mtag : michel_tags)
    {
      ...
      if (mtag_handle.isValid())
      {
        ...
        for (unsigned i = 0; i < mtags.size(); i++)
        {
          ...
          FillMichelTag(mtags[i], srmichel_tags[mtag].back());
        }
      }
    }
    ...
    for (unsigned sliceID = 0; sliceID < slices.size(); sliceID++)
    {
      ...
      const sbn::TPCPMTBarycenterMatch *barycenterMatch = foTPCPMTBarycenterMatch.isValid() ? foTPCPMTBarycenterMatch.at(0).get() : nullptr;

      art::FindOneP<lcvn::Result> foCVNResult =
          FindOnePStrict<lcvn::Result>(sliceList, evt,
                                       fParams.CVNLabel() + slice_tag_suff);
      const lcvn::Result *cvnResult = foCVNResult.isValid() ? foCVNResult.at(0).get() : nullptr;
      ...
      // select slice
      if (!SelectSlice(recslc, fParams.CutClearCosmic()))
        continue;
      ...
      for (size_t iStub = 0; iStub < fmStubs.size(); iStub++)
      {
        ...
        if (!fmStubPFPs.at(iStub).empty())
          thisStubPFP = fmStubPFPs.at(iStub).at(0);
      ...
      if (fParams.FillHits())
      {
        for (size_t iHit = 0; iHit < slcHits.size(); ++iHit)
        {
          ...
          if (!thisParticle.empty() && !thisPoint.empty())
          {
            ...
            {
              art::ServiceHandle<cheat::BackTrackerService> bt_serv;
              auto track_ids = bt_serv->HitToTrackIDEs(clock_data, thisHit);
              float cur_energy = -1;
              for (const auto tid : track_ids)
              {
                if (tid.energy > cur_energy)
                {
                  cur_energy = tid.energy;
                  rec.reco.hit.back().g4id = tid.trackID;
                }
              }
            }
      ...
      for (size_t iPart = 0; iPart < fmPFPart.size(); ++iPart)
      {
        ...
        { // it has a track!
          ...
          {
            art::FindManyP<sbn::crt::CRTHit> CRTT02Hit = FindManyPStrict<sbn::crt::CRTHit>(fmCRTHitMatch.at(iPart), evt, fParams.CRTHitMatchLabel() + slice_tag_suff);
          ...
          if (fmTrackHit.isValid())
          {
            if (!isRealData)
            {
              ...
              if (mc_particles.isValid() && fParams.FillTrackCaloTruth())
                FillTrackCaloTruth(id_to_ide_map, *mc_particles, *geom, wireReadout, clock_data, sce, trk);
            }
          }
        } // thisTrack exists
        ...
        { // it has shower!
          ...
          {
            if (!isRealData)
              FillShowerTruth(fmShowerHit.at(iPart), id_to_hit_energy_map, true_particles, clock_data, shw);
          }
    ...
    rec.michel_tags_deco = srmichel_tags[michel_tags[0]];
    rec.michel_tags_raw = srmichel_tags[michel_tags[1]];
  ...
  void CAFMaker::endJob()
  {
    ...
    try
    {
      ...
      for (auto it : strs)
        metamap[it.first] = "\"" + it.second + "\"";
      for (auto it : ints)
        metamap[it.first] = std::to_string(it.second);
      for (auto it : doubles)
        metamap[it.first] = std::to_string(it.second);
      for (auto it : objs)
        metamap[it.first] = it.second;
    ...
    if (fFile)
      AddMetadataToFile(fFile, metamap);
    if (fParams.CreateBlindedCAF() && fFileb)
      AddMetadataToFile(fFileb, metamap);
    if (fParams.CreateBlindedCAF() && fFilep)
      AddMetadataToFile(fFilep, metamap);
    if (fFlatFile)
      AddMetadataToFile(fFlatFile, metamap);
    if (fParams.CreateBlindedCAF() && fFlatFileb)
      AddMetadataToFile(fFlatFileb, metamap);
    if (fParams.CreateBlindedCAF() && fFlatFilep)
      AddMetadataToFile(fFlatFilep, metamap);
  }
